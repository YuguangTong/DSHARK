...
contains
  ...
  subroutine solve_disp(Nspecies_in, theta_in, delta_in, &
    ...
    ! output params
    !--------------------------------------------------------------------------------
    real*8, intent(out) :: krange(nk)
    complex*16, intent(out) :: solution(nk)
    ...
    ! write(*,'(A12, F12.4)') 'theta:', theta_in
    ! write(*,'(A12, E6.3)') 'delta:', delta_in
    ! write(*,'(A12,F6.3,A10,F6.3)') 'kstart:', kstart, '    kend:', kend
    ! write(*,'(A12,I8)') 'nk:', nk

    ! print *, 'q_in:', q_in
    ! print *, 'mu_in:', mu_in
    ! print *, 'dens_in:', dens_in
    ! print *, 'beta_para_in:', beta_para_in
    ! print *, 'beta_perp_in:', beta_perp_in
